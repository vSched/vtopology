...
//    	}
	...
	std::ifstream vcapFile("/home/ubuntu/banlist/vcap_strag.txt");
    std::string vcapContent;
    if (vcapFile.is_open()) {
        ...
        std::cout << "Unable to open file /home/ubuntu/banlist/vcap_strag.txt for reading" << std::endl;
        return;
    }

    std::istringstream iss(vcapContent);
    std::vector<int> vcapNumbers;
    int num;
    while (iss >> num) {
        if (banset.find(num) == banset.end()) {
            vcapNumbers.push_back(num);
        }
    }

    std::ofstream vcapOutFile("/home/ubuntu/banlist/vcap_strag.txt");
    if (vcapOutFile.is_open()) {
        for (int i = 0; i < vcapNumbers.size(); ++i) {
            vcapOutFile << vcapNumbers[i];
            if (i < vcapNumbers.size() - 1) {
                vcapOutFile << " ";
            }
        }
        vcapOutFile.close();
    } else {
        std::cout << "Unable to open file /home/ubuntu/banlist/vcap_strag.txt for writing" << std::endl;
    }
}
